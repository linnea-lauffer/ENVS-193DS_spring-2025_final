...
sst_clean <- sst |> 
  ...
  filter(year %in% 2018:2023) |> # keep data from 2018-2023
  group_by(year, month) |> # group by year and month
  summarise(mean_monthly_sst = mean(temp)) |> # calculate mean temp for each group
  arrange(desc(year), match(month, month.abb)) |> # arrangeby descending year and month order
  mutate(year = factor(year, levels = sort(unique(year)))) # year treated as factor 
